...
import { githubSignInUrl, signInWithGoogle } from "../apis/auth.ts";
    ...
    <div className="space-y-3">
      <div className="relative">
        ...
        <div className="relative flex justify-center">
          <span className="bg-canvas/95 px-3 text-xs uppercase tracking-wide text-subtle">
            Or continue with
          </span>
        </div>
      </div>
...
function GoogleSignIn() {
  ...
  const googleLogin = useGoogleLogin({
    ...
    onSuccess: async (response) => {
      ...
      setBusy(true);
      try {
        const profile = await signInWithGoogle(response.code);
        setSession(profile);
